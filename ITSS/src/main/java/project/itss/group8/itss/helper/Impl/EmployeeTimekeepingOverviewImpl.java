...
public class EmployeeTimekeepingOverviewImpl implements IEmployeeTimekeepingOverview {
    ...
    public ObservableList<TimekeepingOverview> getTimekeepingByMonth(LocalDate time, LocalDateTime start,
        ...
        List<LogInfor> logInfors = logInforRepository.getLogInforByMonth(time.getMonth().getValue(), time.getYear(),
                Constant.employee.getID());
        ...
        for (int i = 0; i < logInfors.size(); i++) {
            ...
            t1 = new TimekeepingOverview(date1.getDayOfMonth(), getStartTime(date1), null,
                    getComeLateTime(date1, start),
                    null);
